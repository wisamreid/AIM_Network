...
% define the integer numbner of harmonics to attend. If f0 only, n = 1
n = 1;
...
for trials = 20
    for attendTarget = attendTargets
        % ===== f0 information =====
...
%             paramH.IE_BW= 0.07; 
        ...
        if attention
            ...
            simLen = length(spk_IC);
            time_end = floor(simLen/fs*1000)-3;
            dt = 1/fs*1000;
            I2input = ones(length(dt:dt:time_end),nf); % I2 on = attention off
        end
        ...
        if attention
            ...
            save([dataFolder filesep sprintf('spks_t%02i_%s.mat',trials,'none')],'spks','cf');
        end
    end
end
