...
%         fileloc = 'Z:\eng_research_hrc_binauralhearinglab\kfchou\ActiveProjects\Top-Down AIM network\Sims\broad mode - staggered talkers 64Chan500-20000hz\CRM talker4';
        chans = {'00','90','-90'}; %input locations
        refLabels = {'S2','S3','S1'}; %talker labels
        swav123 = [];
        spk123 = [];
        for i = 1:3
            ic(i).wav = audioread([fileloc filesep '0_masker_set_' num2str(talkerSet,'%02i') '_pos_' chans{i} 'degAz_target_conv.wav']);
            ic(i).data = load([fileloc filesep '0_masker_set_' num2str(talkerSet,'%02i') '_pos_' chans{i} 'degAz_SpkIC.mat']);
...
%         fileloc = 'Z:\eng_research_hrc_binauralhearinglab\kfchou\ActiveProjects\Top-Down AIM network\Sims\broad mode - staggered talkers 64Chan500-20000hz\CRM talker4';
        chans = {'00'};
        tgtwav = audioread([fileloc filesep '0_masker_set_' num2str(talkerSet,'%02i') '_pos_' chans{1} 'degAz_target_conv.wav']);
        spkData = load([fileloc filesep '0_masker_set_' num2str(talkerSet,'%02i') '_pos_' chans{1} 'degAz_SpkIC.mat']);
