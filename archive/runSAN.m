...
% specify spatial channel(s) to target;
% st neurons would only target this specific spatial channel
targetSpatialChan = 3;
stSpatialChanTargetNetconSmall = diag(zeros(1,nLocs));
stSpatialChanTargetNetconSmall(targetSpatialChan,targetSpatialChan) = 1; 
stSpatialChanTargetNetconRep = repmat({stSpatialChanTargetNetconSmall},1,nFreqs);
stSpatialChanTargetNetconGroupByLoc = blkdiag(stSpatialChanTargetNetconRep{:});
stSpatialChanTargetNetcon = regroup(stSpatialChanTargetNetconGroupByLoc, [nFreqs,nLocs]); %same as default
%stInitNetcon = eye(N_pre,N_post); 
...
s.connections(end).parameters={'IappI2',0.02,'IappI2Varied',0.02,'nFreq',nFreqs,'stSubNetcon',stSpatialChanTargetNetcon,'simLen',simLen+200};
...
if options.plotRasters
    ...
    for j = 1:length(data)
        ...
        subplot(5,5,3)
        plotSpikeRasterFs(logical([data(j).C_V_spikes])', 'PlotType','vertline', 'Fs',fs);
        xlim([0 time_end+200])
        ylabel('C spikes');
        for i = 1:5
