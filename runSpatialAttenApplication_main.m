...
% for plotting results, see fig6E.m
...
% ================ parameters ==============
attend = 0; % attention on or off?
attendTarget = 'T'; %T, or M?
attendAzs = 0; % location of talkers

azList = 0:15:90; % must match spatial channels from IC model
nLocs = length(azList);
nFreqs = 64;
trials = 1:20;
...
i = 1;
for attendAz = attendAzs
    if strcmp(attendTarget,'T')
        [~,tgtTDidx] = min(abs(azList - 0));
    elseif strcmp(attendTarget,'M')
    ...
    for trial = trials
        ...
        % for trial = trials
            % ------------ load spk_IC here -----------------
%             spks = load([stimuliRoot filesep sprintf('IC_spks_m%i_set_%02i.mat',attendAz,trial)]);
        ...
        saveas(h,[dataFolder filesep sprintf('Cspikes trial%d attend%d azimuth%d%s.pdf',trial,attend,attendAz,suffix)])
        save([dataFolder filesep sprintf('Cspikes trial%d attend%d% azimuth%d%s.mat',trial,attend,attendAz,suffix)],'Cspikes');
    end
end
return;
