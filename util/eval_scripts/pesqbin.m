...
%
% 2018/05/03 KennyFChou - updated wavread/wavwrite to audioread/audiowrite
...
function [ scores ] = pesqbin_windows( reference, degraded, fs, mode, binary )
    ...
    switch lower( mode )
        ...
        case { 'wb', '+wb', 'wideband', '+wideband' }
            ...
            command = sprintf( 'pushd %%CD%% && cd %s && %s %s %s +%i +wb && popd', tmpdir, binary, file.reference, file.degraded, fs);
