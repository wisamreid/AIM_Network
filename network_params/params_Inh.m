% Parameters to induce weakening of inhibitory region in frequency
% dependent PSTHs. Iintrinsic firing occurs in E neurons
% 
...
varies(end+1).conxn = 'IC->S';
varies(end).param = 'gSYN';
varies(end).range = 4.5;
...
varies(end).param = 'gSYN';
varies(end).range = 3 %1;

varies(end+1).conxn = 'S->E';
varies(end).param = 'gSYN';
varies(end).range = 6.5;

varies(end+1).conxn = 'S->E';
varies(end).param = 'tauD';
varies(end).range = 2;
...
varies(end).param = 'gSYN';
varies(end).range = 5; %4;

varies(end+1).conxn = 'E';
varies(end).param = 'Itonic';
varies(end).range = 28; %19;
